...
class FeedController extends Controller
{
    public function rss()
    {
        ...
        echo '<rss version="2.0"><channel><title>' . htmlspecialchars(Settings::get('site_name_ar', 'منصة الأخبار التقنية'), ENT_XML1, 'UTF-8') . '</title><link>' . htmlspecialchars(app_url(), ENT_XML1, 'UTF-8') . '</link><description>آخر الأخبار التقنية</description>';
    ...
    public function rssByCategory($slug)
    {
        ...
        header('Content-Type: application/rss+xml; charset=utf-8');
        echo '<?xml version="1.0" encoding="UTF-8"?>';
        echo '<rss version="2.0"><channel><title>' . htmlspecialchars($category['name'] . ' - ' . Settings::get('site_name_ar', 'منصة الأخبار التقنية'), ENT_XML1, 'UTF-8') . '</title><link>' . htmlspecialchars(app_url(), ENT_XML1, 'UTF-8') . '</link><description>آخر الأخبار في قسم ' . htmlspecialchars($category['name'], ENT_XML1, 'UTF-8') . '</description>';
        foreach ($articles as $article) {
            $link = app_url('article/' . $article['slug']);
            echo '<item><title>' . htmlspecialchars($article['title'], ENT_XML1, 'UTF-8') . '</title><link>' . htmlspecialchars($link, ENT_XML1, 'UTF-8') . '</link><guid>' . htmlspecialchars($link, ENT_XML1, 'UTF-8') . '</guid><description><![CDATA[' . ($article['excerpt'] ?? mb_substr(strip_tags($article['content']), 0, 240)) . ']]></description><pubDate>' . date(DATE_RSS, strtotime($article['published_at'] ?: $article['created_at'])) . '</pubDate></item>';
        }
