...
class AiChatAssistant
{
    ...
    public static function ask($question, array $history = [], array $options = [])
    {
        ...
        if (!empty($context['text'])) {
            $messages[] = ['role' => 'system', 'content' => 'محتوى المنصة المتاح للإجابة (إن وجد مطابقاً):' . "\n" . mb_substr($context['text'], 0, 7000, 'UTF-8')];
        }
    ...
    {
        $siteName = class_exists('Settings') ? Settings::get('site_name_ar', 'المنصة') : 'المنصة';
        $toneLine = [
            'formal'     => 'كن رسمياً وموجزاً: لغة دقيقة، جُمل واضحة، بلا مزاح أو مصطلحات عامية.',
            'friendly'   => 'كن ودوداً وخفيف الظل: نبرة ودّية قريبة من المستخدم مع بقاء المحتوى مفيداً ودقيقاً.',
            'educational'=> 'كن معلّماً مبسّطاً: اشرح المفاهيم بلغة سهلة تناسب الزائر غير المختص مع أمثلة عملية قصيرة.',
            'balanced'   => 'كن متوازناً: واضحاً ومرتّباً وسهل العبارة، ودقيقاً ومحترفاً في آن واحد.',
        ];
        return "أنت «المساعد الذكي لـ {$siteName}»."
            . "\n" . ($toneLine[$tone] ?? $toneLine['balanced'])
            . "\n" . "مهمتك الأساسية: مساعدة الزوار والإجابة عن استفساراتهم بالاعتماد على محتوى ومعلومات {$siteName}، وتوجيههم للأقسام والروابط المفيدة."
            . "\n" . "إذا توفر محتوى مرفق (`محتوى المنصة المتاح للإجابة`) اعتمد عليه بدقة وأشر للروابط المناسبة بصيغة: [عنوان الصفحة أو المقال](/article/اسم-الرابط)."
            . "\n" . "أجب دائماً باللغة العربية الفصحى الواضحة والمنسقة باستخدام Markdown النظيف (عناوين، نقاط، خط عريض)."
            . "\n" . "كن مهذباً، نافعاً، ومحترفاً في كافة الإجابات.";
    }
    ...
    private static function callGemini($apiKey, $model, array $messages, array $cfg)
    {
        ...
        foreach ($chatParts as $i => $cpart) {
            if ($cpart['role'] === 'model' && $i === 0) {
                $chatParts[$i]['parts'][0]['text'] = 'حسناً، أسألني عن المحتوى المنشور في المنصة.';
            }
        }
